...
# Confirmation Modal
CONFIRMATION_MODAL_CSS = (_UI_DIR / "confirmation_modal.css").read_text(
    encoding="utf-8"
)
...
CONFIRMATION_MODAL_HTML = (_UI_DIR / "confirmation_modal.html").read_text(
    encoding="utf-8"
)
...
def create_qwen_advanced_params(
    ...
    with gr.Accordion("Advanced Parameters", open=False, visible=visible) as accordion:
        ...
        if include_emotion:
            emotion_choices = (
                get_emotion_choices(emotions_dict) if emotions_dict else []
            )
            ...
            with gr.Row(visible=emotion_visible) as emotion_buttons_row:
                ...
                components["delete_emotion_btn"] = gr.Button(
                    "Delete", size="sm", scale=1
                )
    ...
    if include_emotion:
        if not shared_state or "_active_emotions" not in shared_state:
            raise ValueError(
                "shared_state with '_active_emotions' key is required when include_emotion=True"
            )
...
def create_emotion_intensity_slider(
    initial_intensity=1.0, label="Emotion Intensity", visible=True
):
    """
...
def create_luxtts_advanced_params(
    ...
    Args:
        ...
        initial_device: Device selection (auto/cpu/cuda)
        initial_threads: CPU threads when running on CPU
        visible: Make accordion visible
    ...
    with gr.Accordion(
        "LuxTTS Advanced Parameters", open=False, visible=visible
    ) as accordion:
        with gr.Row():
            components["num_steps"] = gr.Slider(
                minimum=1,
        ...
        with gr.Row():
            components["device"] = gr.Dropdown(
                choices=["auto", "cpu", "cuda"],
                value=initial_device,
                label="Device",
                info="Auto selects CUDA when available",
            )
            components["threads"] = gr.Number(
                value=initial_threads,
                precision=0,
                label="CPU Threads",
                info="Used only when Device=cpu",
            )
