...
class ConversationTool(Tool):
    ...
    def create_tool(cls, shared_state):
        ...
        create_vibevoice_advanced_params = shared_state[
        ...
        create_emotion_intensity_slider = shared_state[
        ...
        with gr.TabItem("Conversation"):
            ...
            conversation_first_sample = (
                conversation_available_samples[0]
                if conversation_available_samples
                else None
            )
            ...
            with gr.Row():
                ...
                with gr.Column(scale=2):
                    ...
                    components["qwen_base_voices_section"] = gr.Column(
                    ...
                    with components["qwen_base_voices_section"]:
                        ...
                        components["refresh_qwen_samples_btn"] = gr.Button(
                    ...
                    with components["luxtts_voices_section"]:
                        ...
                        components["refresh_luxtts_samples_btn"] = gr.Button(
                            "Refresh Voice Samples", size="md"
                        )

                    # VibeVoice voice sample selectors
                    components["vibevoice_voices_section"] = gr.Column(
                    ...
                    with components["vibevoice_voices_section"]:
                        ...
                        components["refresh_conv_samples_btn"] = gr.Button(
                            "Refresh Voice Samples", size="md"
                        )
                ...
                with gr.Column(scale=1):
                    ...
                    components["qwen_custom_settings"] = gr.Column(
                        visible=is_qwen_custom
                    )
                    ...
                    initial_lang_value = (
                        _user_config.get("language", "Auto")
                        if is_qwen_initial
                        else "Auto"
                    )
                    with gr.Column():
                    ...
                    components["qwen_pause_controls"] = gr.Accordion(
                        "Pause Controls",
                        open=False,
                    ...
                    with components["luxtts_settings"]:
                        ...
                        components["lux_conv_guidance_scale"] = lux_conv_params[
                            "guidance_scale"
                        ]
                        components["lux_conv_rms"] = lux_conv_params["rms"]
                        components["lux_conv_ref_duration"] = lux_conv_params[
                            "ref_duration"
                        ]
                        components["lux_conv_return_smooth"] = lux_conv_params[
                            "return_smooth"
                        ]
                    ...
                    with components["vibevoice_settings"]:
                        ...
                        vv_conv_params = create_vibevoice_advanced_params(
                            initial_num_steps=20, initial_cfg_scale=3.0, visible=True
                        )
                        ...
                        components["vv_conv_repetition_penalty"] = vv_conv_params[
                            "repetition_penalty"
                        ]
                        components["vv_conv_temperature"] = vv_conv_params[
                            "temperature"
                        ]
                    ...
                    components["qwen_conv_advanced"] = gr.Column(
                        visible=(is_qwen_custom or is_qwen_base)
                    )
                    with components["qwen_conv_advanced"]:
                        ...
                        with components["conv_emotion_intensity_row"]:
                            components["conv_emotion_intensity"] = (
                                create_emotion_intensity_slider(
                        ...
                        components["qwen_conv_do_sample"] = qwen_conv_params[
                            "do_sample"
                        ]
                        components["qwen_conv_temperature"] = qwen_conv_params[
                            "temperature"
                        ]
                        components["qwen_conv_top_k"] = qwen_conv_params["top_k"]
                        components["qwen_conv_top_p"] = qwen_conv_params["top_p"]
                        components["qwen_conv_repetition_penalty"] = qwen_conv_params[
                            "repetition_penalty"
                        ]
                        components["qwen_conv_max_new_tokens"] = qwen_conv_params[
                            "max_new_tokens"
                        ]

                    # Shared settings
                    components["conv_generate_btn"] = gr.Button(
    ...
    def setup_events(cls, components, shared_state):
        ...
        def generate_conversation_handler(
            ...
            try:
                ...
                if not lines:
                    return (
                        None,
                        "❌ No valid conversation lines found. Use format: [N]: Text",
                    )
                ...
                for i, (speaker, text) in enumerate(lines):
                    ...
                    if pause_comma > 0:
                        clean_text = re.sub(
                            r",(?!\d)", f", [break={pause_comma}]", clean_text
                        )
                    if pause_question > 0:
                        clean_text = re.sub(
                    ...
                    if pause_hyphen > 0:
                        clean_text = re.sub(
                            r"-(?!\d)", f"- [break={pause_hyphen}]", clean_text
                        )
                    ...
                    if style_instruct:
                        ...
                        progress(
                            progress_val, desc=f"Line {i + 1}/{len(lines)} ({speaker})"
                        )
        ...
        def generate_conversation_base_handler(
            conversation_data,
            voice_samples_dict,
            pause_linebreak,
            pause_period,
            ...
            try:
                ...
                if not lines:
                    return (
                        None,
                        "❌ No valid conversation lines found. Use format: [N]: Text (N=1-8)",
                    )
                ...
                for i, (speaker_key, voice_sample_path, ref_text, text) in enumerate(
                    lines
                ):
                    progress_val = 0.1 + (0.8 * i / len(lines))
                    ...
                    if emotion_key and emotion_key in _active_emotions:
                        ...
                        line_temp = max(
                            ...
                            min(
                                2.0,
                                temperature + (adjustments["temp"] * emotion_intensity),
                            ),
                        )
                        line_top_p = max(
                            0.0,
                            min(
                                1.0, top_p + (adjustments["top_p"] * emotion_intensity)
                            ),
                        )
                        line_rep_pen = max(
                            1.0,
                            min(
                                2.0,
                                repetition_penalty
                                + (adjustments["penalty"] * emotion_intensity),
                            ),
                        )
                        progress(
                            progress_val,
                            desc=f"Line {i + 1}/{len(lines)} ({speaker_key}) [{emotion_key}]",
                        )
                    else:
                        progress(
                            progress_val,
                            desc=f"Line {i + 1}/{len(lines)} ({speaker_key})",
                        )
                    ...
                    if get_or_create_voice_prompt:
                        voice_prompt = get_or_create_voice_prompt(
                            model, speaker_key, voice_sample_path, ref_text, model_size
                        )
                ...
                print(
                    f"Error in generate_conversation_base_handler:\n{traceback.format_exc()}"
                )
                return None, f"❌ Error generating conversation: {str(e)}"

        def generate_vibevoice_longform_handler(
            script_text,
            voice_samples_dict,
            model_size,
            cfg_scale,
            ...
            try:
                ...
                # Import processor
                from vibevoice_tts.processor.vibevoice_processor import (
                    VibeVoiceProcessor,
                )
                import warnings
                import logging
                    ...
                    if line.startswith("[") and "]" in line:
                        ...
                        if bracket_content.isdigit():
                            ...
                            if text:
                                ...
                                lines.append(
                                    (f"Speaker{wrapped_num}", text, speaker_num)
                                )
                ...
                for i in range(1, 5):
                    ...
                    if (
                        speaker_key in voice_samples_dict
                        and voice_samples_dict[speaker_key]
                    ):
                        sample_data = voice_samples_dict[speaker_key]
                        wav_path = (
                            sample_data["wav_path"]
                ...
                if not available_samples:
                    return (
                        None,
                        "❌ Please provide at least one voice sample (Speaker1).",
                    )
                ...
                for speaker, text, original_num in lines:
                    if speaker in speaker_to_sample:
                        ...
                        formatted_lines.append(
                            f"Speaker {vv_speaker_num}: {clean_text}"
                        )
                ...
                print(
                    f"Error in generate_vibevoice_longform_handler:\n{traceback.format_exc()}"
                )
                return None, f"❌ Error generating conversation: {str(e)}"
        ...
        def generate_luxtts_conversation_handler(
            ...
            try:
                                ...
                                if speaker_key in voice_samples_dict and text:
                                    ...
                                    lines.append(
                                        (
                                            speaker_key,
                                            sample_data["wav_path"],
                                            sample_data.get("name", speaker_key),
                                            text,
                                            sample_data.get("ref_text", ""),
                                        )
                                    )
                ...
                if not lines:
                    return (
                        None,
                        "Error: No valid conversation lines found. Use format: [N]: Text (N=1-8)",
                    )
                ...
                for i, (
                    ...
                    progress(
                        progress_val, desc=f"Line {i + 1}/{len(lines)} ({speaker_key})"
                    )

                    audio_data, audio_sr, was_cached = (
                ...
                print(
                    f"Error in generate_luxtts_conversation_handler:\n{traceback.format_exc()}"
                )
        ...
        def unified_conversation_generate(
            ...
            if model_type == "Qwen CustomVoice":
                ...
                return generate_conversation_handler(
                    script,
                    qwen_custom_pause_linebreak,
                    qwen_custom_pause_period,
                    qwen_custom_pause_comma,
                ...
                return generate_conversation_base_handler(
                    script,
                    voice_samples,
                    qwen_base_pause_linebreak,
                    qwen_base_pause_period,
                ...
                return generate_luxtts_conversation_handler(
                    script,
                    voice_samples,
                    lux_pause_linebreak,
                    seed,
                ...
                return generate_vibevoice_longform_handler(
                    script,
                    voice_samples,
                    vv_size,
                    vv_cfg,
        ...
        components["conv_generate_btn"].click(
            ...
            inputs=[
                components["conv_model_type"],
                components["conversation_script"],
                # Qwen CustomVoice
                components["conv_pause_linebreak"],
                components["conv_pause_period"],
                components["conv_pause_comma"],
                components["conv_pause_question"],
                components["conv_pause_hyphen"],
                components["conv_model_size"],
                # Qwen Base
                components["qwen_voice_sample_1"],
                components["qwen_voice_sample_2"],
                components["qwen_voice_sample_3"],
                components["qwen_voice_sample_4"],
                components["qwen_voice_sample_5"],
                components["qwen_voice_sample_6"],
                components["qwen_voice_sample_7"],
                components["qwen_voice_sample_8"],
                components["conv_pause_linebreak"],
                components["conv_pause_period"],
                components["conv_pause_comma"],
                components["conv_pause_question"],
                components["conv_pause_hyphen"],
                components["conv_base_model_size"],
                # Shared Qwen
                components["conv_language"],
                components["conv_seed"],
                components["conv_emotion_intensity"],
                # Qwen advanced params
                components["qwen_conv_do_sample"],
                components["qwen_conv_temperature"],
                components["qwen_conv_top_k"],
                components["qwen_conv_top_p"],
                components["qwen_conv_repetition_penalty"],
                components["qwen_conv_max_new_tokens"],
                # VibeVoice
                components["voice_sample_1"],
                components["voice_sample_2"],
                components["voice_sample_3"],
                components["voice_sample_4"],
                components["longform_model_size"],
                components["longform_cfg_scale"],
                # VibeVoice advanced params
                components["vv_conv_num_steps"],
                components["vv_conv_do_sample"],
                components["vv_conv_temperature"],
                components["vv_conv_top_k"],
                components["vv_conv_top_p"],
                components["vv_conv_repetition_penalty"],
                # LuxTTS
                components["luxtts_voice_sample_1"],
                components["luxtts_voice_sample_2"],
                components["luxtts_voice_sample_3"],
                components["luxtts_voice_sample_4"],
                components["luxtts_voice_sample_5"],
                components["luxtts_voice_sample_6"],
                components["luxtts_voice_sample_7"],
                components["luxtts_voice_sample_8"],
                components["luxtts_pause_linebreak"],
                components["lux_conv_num_steps"],
                components["lux_conv_t_shift"],
                components["lux_conv_speed"],
                components["lux_conv_guidance_scale"],
                components["lux_conv_rms"],
                components["lux_conv_ref_duration"],
                components["lux_conv_return_smooth"],
                # Shared
                components["conv_seed"],
            ],
        ...
        def toggle_conv_ui(model_type):
            ...
            return {
                ...
                components["conv_emotion_intensity_row"]: gr.update(
                    visible=is_qwen_base
                ),
        ...
        components["conv_model_type"].change(
            ...
            outputs=[
                components["qwen_speaker_table"],
                components["qwen_base_voices_section"],
                components["luxtts_voices_section"],
                components["vibevoice_voices_section"],
                components["qwen_custom_settings"],
                components["qwen_base_settings"],
                components["qwen_pause_controls"],
                components["conv_emotion_intensity_row"],
                components["qwen_conv_advanced"],
                components["luxtts_settings"],
                components["vibevoice_settings"],
                components["qwen_custom_tips"],
                components["qwen_base_tips"],
                components["vibevoice_tips"],
