...
def _sample_filename(idx):
    """Return the .pt filename for a sample index.

    Supports both naming schemes:
    - int idx  → legacy sequential format: sample_000042.pt
    - string   → stem-based format:        my_clip.pt
    """
    if isinstance(idx, int):
        return f"sample_{idx:06d}.pt"
    return f"{idx}.pt"


def build_speaker_map(index_paths, data_dirs):
    """Map speaker → [(data_dir, sample_id)] from index file(s).

    Field 0 of the `~`-delimited row is used as the sample identifier.
    When it parses as an int the legacy `sample_{idx:06d}.pt` filename
    scheme is used; when it is a string stem (new scheme) the file is
    looked up directly as `{stem}.pt`.
    """
        ...
        with open(index_path) as f:
            for line_num, line in enumerate(f):
                ...
                try:
                    ...
                    idx = parts[0]  # string stem — new naming scheme
                speaker_id = parts[1]
...
class IDLoRADataset(Dataset):
    ...
    def __init__(self, speaker_map):
        ...
        for speaker, entries in speaker_map.items():
            ...
            for data_dir, idx in entries:
                audio_path = Path(data_dir) / "audio_latents" / _sample_filename(idx)
                cond_path = Path(data_dir) / "conditions" / _sample_filename(idx)
                if not audio_path.exists():
                    skipped_audio += 1
                    if skipped_audio <= 3:
                        logging.warning(f"  Missing audio_latent: {audio_path}")
                elif not cond_path.exists():
                    skipped_cond += 1
                    if skipped_cond <= 3:
                        logging.warning(f"  Missing conditions: {cond_path}")
                else:
                    valid.append((data_dir, idx))
            if skipped_audio:
                logging.warning(f"Speaker '{speaker}': {skipped_audio} missing audio_latent files")
            if skipped_cond:
                logging.warning(f"Speaker '{speaker}': {skipped_cond} missing conditions files")
            if len(valid) >= 2:
                self.speaker_map[speaker] = valid
            else:
                logging.warning(f"Speaker '{speaker}': only {len(valid)} valid samples (need >= 2), skipping")
    ...
    def _load_sample(self, data_dir, idx):
        base = Path(data_dir)
        audio = torch.load(base / "audio_latents" / _sample_filename(idx), weights_only=False)
        ...
        stripped = base / "conditions_stripped" / _sample_filename(idx)
        cond_path = stripped if stripped.exists() else base / "conditions" / _sample_filename(idx)
...
def _finalize_save(output_dir, speaker_slug):
    """Rename PEFT adapter_model.safetensors to the speaker-named live file
    and remove auto-generated PEFT artifacts (README.md).
    Returns the path of the live .safetensors file."""
    for name in ("README.md",):
        p = os.path.join(output_dir, name)
        if os.path.exists(p):
            os.remove(p)
    adapter = os.path.join(output_dir, "adapter_model.safetensors")
    live = os.path.join(output_dir, f"{speaker_slug}_dramabox.safetensors")
    if os.path.exists(adapter):
        if os.path.exists(live):
            os.remove(live)
        os.rename(adapter, live)
    return live
...
def main():
    ...
    # Derive a safe slug from the output folder name for checkpoint filenames
    import re as _re_slug
    speaker_slug = _re_slug.sub(r"[^\w-]", "_", Path(args.output_dir).name).strip("_").lower()
    if is_main:
        import yaml
    ...
    if not speaker_map:
        logging.error(
            "Speaker map is empty. Possible causes:\n"
            "  1. speaker_index.txt keys don't match audio_latent filenames\n"
            "  2. audio_latents/ or conditions/ files are missing\n"
            "  3. All speakers have fewer than 2 valid samples\n"
            f"  data-dir:      {args.data_dir}\n"
            f"  speaker-index: {args.speaker_index}"
        )
        # Log first few lines of speaker_index for diagnosis
        try:
            with open(args.speaker_index[0]) as _f:
                _lines = [l.rstrip() for l in _f.readlines()[:5]]
            logging.error(f"First 5 lines of speaker_index:\n" + "\n".join(_lines))
            # Check if the first expected file exists
            _parts = _lines[0].split("~") if _lines else []
            if _parts:
                _stem = _parts[0]
                _audio_check = Path(args.data_dir[0]) / "audio_latents" / f"{_stem}.pt"
                _cond_check = Path(args.data_dir[0]) / "conditions" / f"{_stem}.pt"
                logging.error(f"  audio_latents/{_stem}.pt exists: {_audio_check.exists()} ({_audio_check})")
                logging.error(f"  conditions/{_stem}.pt exists:    {_cond_check.exists()} ({_cond_check})")
        except Exception as _e:
            logging.error(f"  (could not read speaker_index: {_e})")
        sys.exit(1)
    ...
    for micro_step in range(total_micro_steps):
        ...
        if is_opt_step and step % args.log_every == 0 and is_main:
            ...
            if avg_loss < best_loss:
                best_loss = avg_loss
                old_best = os.path.join(args.output_dir, f"{speaker_slug}_dramabox_best_{best_step:05d}.safetensors")
                best_step = step + args.resume_step_offset
                new_best = os.path.join(args.output_dir, f"{speaker_slug}_dramabox_best_{best_step:05d}.safetensors")
                unwrapped = _unwrap_model_safe(model)
                unwrapped.save_pretrained(args.output_dir)
                live = _finalize_save(args.output_dir, speaker_slug)
                if os.path.exists(live):
                    shutil.copy(live, new_best)
        ...
        if is_opt_step and step % args.save_every == 0 and is_main:
            ...
            save_path = os.path.join(args.output_dir, f"{speaker_slug}_dramabox_{global_step:05d}.safetensors")
            logging.info(f"Saving: {save_path}")
            unwrapped = _unwrap_model_safe(model)
            unwrapped.save_pretrained(args.output_dir)
            live = _finalize_save(args.output_dir, speaker_slug)
            if os.path.exists(live):
                shutil.copy(live, save_path)
